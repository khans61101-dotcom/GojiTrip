...
export default function RouteTimeline() {
  ...
  if (!routeSearch) {
    const sampleStops = [
      { sequence: "01", name: "Kathmandu", type: "source", label: "START", badgeBg: "bg-blue-50 text-blue-600", color: "bg-blue-600", desc: "Capital City & Cultural Heritage Start", dist: "0 km", time: "0h 00m" },
      { sequence: "02", name: "Kurintar (Trishuli River)", type: "rest", label: "FOOD & RAFTING", badgeBg: "bg-orange-50 text-orange-600", color: "bg-orange-500", desc: "Popular highway lunch stop, Cable Car & River Rafting", dist: "85 km", time: "2h 30m" },
      { sequence: "03", name: "Pokhara Lakeside", type: "hotel", label: "POPULAR HUB", badgeBg: "bg-emerald-50 text-emerald-600", color: "bg-emerald-600", desc: "Phewa Lake, Paragliding Tandem Pilots & Luxury Resorts", dist: "200 km", time: "6h 30m" },
      { sequence: "04", name: "Tatopani Hot Springs", type: "rest", label: "REST & RELAX", badgeBg: "bg-purple-50 text-purple-600", color: "bg-purple-600", desc: "Natural Geothermal Springs & Annapurna Trek Junction", dist: "285 km", time: "9h 30m" },
      { sequence: "05", name: "Muktinath Temple", type: "destination", label: "DESTINATION", badgeBg: "bg-red-50 text-red-600", color: "bg-red-500", desc: "Mustang Holy Pilgrimage (Elevation: 3,710m)", dist: "375 km", time: "13h 00m" },
    ];

    return (
      <section
        id="complete-route-analysis"
        className="py-16 md:py-24 bg-slate-50 border-y border-slate-200 text-slate-900"
      >
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="text-center max-w-3xl mx-auto mb-12">
            <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 text-xs font-bold tracking-wider uppercase">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              ✨ AI-POWERED ROUTE INTELLIGENCE
            </span>

            <h2 className="text-3xl md:text-5xl font-extrabold text-slate-950 mt-5 tracking-tight">
              Complete Route Analysis
            </h2>

            <p className="text-sm md:text-base text-slate-600 mt-3 leading-relaxed">
              Below is a sample breakdown for <strong className="text-emerald-700 font-bold">Kathmandu ➔ Pokhara ➔ Mustang Corridor</strong>. Enter your custom starting point and destination above and click <strong className="text-emerald-600">Search Route</strong> to generate your personalized live trip!
            </p>
          </div>

          {/* Interactive Preview Route Timeline */}
          <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-xl space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100">
              <div>
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-emerald-600">
                  Featured Nepal Corridor Preview
                </span>
                <h3 className="text-base sm:text-lg font-extrabold text-slate-900">
                  Kathmandu to Muktinath (Mustang Highway)
                </h3>
              </div>
              <div className="flex items-center space-x-3 text-xs font-semibold text-slate-500">
                <span className="bg-slate-100 px-3 py-1 rounded-full">Distance: 375 km</span>
                <span className="bg-slate-100 px-3 py-1 rounded-full">Duration: ~13 Hours</span>
              </div>
            </div>

            <div className="space-y-6">
              {sampleStops.map((stop, idx) => (
                <div key={idx} className="flex gap-4 items-start">
                  <div className={`w-8 h-8 rounded-full ${stop.color} text-white font-bold text-xs flex items-center justify-center shadow-md flex-shrink-0 mt-1`}>
                    {stop.sequence}
                  </div>
                  <div className="flex-1 bg-slate-50/80 p-4 rounded-2xl border border-slate-200/80 hover:border-emerald-400 transition-colors">
                    <div className="flex items-center justify-between flex-wrap gap-2 mb-1">
                      <h4 className="font-extrabold text-sm text-slate-900">
                        {stop.name}
                      </h4>
                      <div className="flex items-center space-x-2">
                        <span className={`text-[9px] font-extrabold uppercase tracking-wider px-2 py-0.5 rounded-full ${stop.badgeBg}`}>
                          {stop.label}
                        </span>
                    ...
                    <p className="text-xs text-slate-600">{stop.desc}</p>
                    
                    {/* Quick Category Buttons */}
                    <div className="flex flex-wrap gap-2 mt-3 pt-2 border-t border-slate-200/60">
                      {actionCategories.slice(0, 5).map((cat) => {
                        const Icon = cat.icon;
                        return (
                          <a
                            key={cat.key}
                            href={`${cat.path}?location=${encodeURIComponent(stop.name)}`}
                            className="px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-[10px] font-semibold text-slate-600 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-300 transition-colors flex items-center space-x-1"
                          >
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center space-x-3">
                <RouteIcon className="w-6 h-6 text-emerald-600 flex-shrink-0" />
                <div>
                  <div className="text-xs font-bold text-slate-900">Ready to plan your own route?</div>
                  <div className="text-[11px] text-slate-600">Select any source & destination in the search box above.</div>
                </div>
              </div>
              <button
                type="button"
                ...
                className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs rounded-xl shadow-md transition-all whitespace-nowrap"
              >
                Search Your Custom Route ➔
              </button>
            </div>
          </div>
        </div>
