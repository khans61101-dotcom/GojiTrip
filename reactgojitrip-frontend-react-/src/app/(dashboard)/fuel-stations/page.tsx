...
export default function AdminFuelStationsPage() {
  ...
  const refreshData = () => {
    const data = cmsStore.getFuelStations().filter((s) => !String(s.id).startsWith("fuel-"));
    setStations(data);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const payload: FuelStationEntry = {
      id: editingStation?.id || `fuel_${Date.now()}`,
      name: formData.name || "",
  ...
  const filteredStations = stations.filter((s) => {
    ...
    const matchesType =
      selectedType === "All Types" || s.stationType === selectedType;
    const matchesStatus =
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto text-slate-100">
          ...
          >
            <div>
              <div className="relative h-44 w-full bg-slate-800">
                ...
                />
                <span className="absolute top-3 left-3 px-3 py-1 bg-slate-955/80 backdrop-blur-md text-slate-200 text-[10px] font-extrabold rounded-full flex items-center space-x-1 border border-slate-800">
              ...
              <div className="p-5 space-y-3">
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {station.hasEvFastCharger && (
                    <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 text-[10px] font-bold rounded-lg flex items-center space-x-1">
                      <Zap className="w-3 h-3" />
                      <span>EV Fast Charger</span>
                    </span>
                  )}
                  {station.hasConvenienceStore && (
                    <span className="px-2 py-0.5 bg-blue-500/10 text-blue-400 border border-blue-500/30 text-[10px] font-bold rounded-lg flex items-center space-x-1">
                      <Store className="w-3 h-3" />
                      <span>Mart</span>
                    </span>
                  )}
                  {station.hasRepairShop && (
                    <span className="px-2 py-0.5 bg-purple-500/10 text-purple-400 border border-purple-500/30 text-[10px] font-bold rounded-lg flex items-center space-x-1">
            ...
            <div className="p-4 bg-slate-950/60 border-t border-slate-800 flex items-center justify-between gap-2">
              <select
                ...
                onChange={(e) => cmsStore.updateStatus("fuel", station.id, e.target.value as ApprovalStatus)}
      ...
      {filteredStations.length === 0 && (
        <div className="bg-slate-900 rounded-3xl p-12 text-center border border-slate-800 text-slate-400">
          No fuel or EV stations found matching your search filters.
        </div>
      )}
